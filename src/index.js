...
d.addEventListener("click", async e => {
  ...
  if (e.target.matches(".delete")) {
    let isDelete = confirm(`¿Estás seguro de que quieres eliminar la tarea ${e.target.dataset.id}?`);
    ...
    if (isDelete) {
      ...
      try {
        ...
        let res = await axios(`http://localhost:3000/task/${e.target.dataset.id}`, options);
        let json = await res.data;
